...
//        ]);
        ...
        if(empty($exist)){  // 没有重复订单
            ...
            $array = array (
                'order_num' => $order_num , 'order_good_tag' => $request->input("tag" , '1') , 'order_good_id' => $request->input("good_id" , '1') , 'order_good_money' => $request->input("good_money" , '10') , 'order_good_num' => $request->input("good_num" , '1') , 'order_user_id' => $request->input("user_id" , session("user_id" , 1)) , 'order_user_address' => $address->address_city ,  // 添加地址城市
                ...
                )) , 'order_time' => time() , 'order_pay_tag' => 0 , 'order_pay_money' => $request->input("pay_money" , 10) , 'order_pay_logistics' => $request->input('pay_logistics' , "哈哈哈哈") ,
            );
...
//        ]);
        ...
        if(empty($exist)){
            $order = [
                'order_num' => $order_num , 'order_user_id' => session('user_id' , 1) ,                 // 电话号码
                'order_adult' => $request->input('adult' , 5) , 'order_child' => $request->input('child' , 5) , 'order_tel' => $request->input('tel' , 1828888888) , 'order_name' => $request->input('name' , '哈哈哈') , 'order_remark' => $request->input('remark' , '备注') , 'order_time' => $_SERVER['REQUEST_TIME'] , 'order_out_of_time' => $_SERVER['REQUEST_TIME'] + 3000 ,    // 过期时间
...
//        ]);
        ...
        if(empty($order)){
            $hotel = array (
                'order_num' => $order_num , 'order_time_start' => strtotime($request->input("time_start" , "2017/8/21 9:19")) , 'order_time_live' => strtotime($request->input('time_live' , "2017/8/29 9:19")) , 'order_room_num' => intval($request->input("room_num" , 3)) , 'order_name' => $request->input("name" , "啊哈哈哈") , 'order_telephone' => $request->input("telephone" , "18282222222") , 'order_remark' => $request->input('remark' , '备注') , 'order_hotel_id' => intval($request->input('hotel_id' , 1)) , 'order_room_id' => intval($request->input('room_id' , 1)) , 'order_pay' => intval($request->input('pay' , 100)) , 'order_price' => intval($request->input('price' , 100)) , 'order_tag' => 0 , 'order_time' => time() , 'order_user_id' => intval(session('user_id' , 1))
            );
